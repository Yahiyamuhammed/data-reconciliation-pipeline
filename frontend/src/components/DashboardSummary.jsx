  ...
  return (
    <div style={{ marginBottom: '2rem' }}>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ border: '1px solid #ccc', padding: '1rem', borderRadius: '4px', flex: 1 }}>
          <h3>Revenue Discrepancies Detected</h3>
      ...
      <div style={{ background: '#f9f9f9', padding: '1.5rem', borderRadius: '6px', borderLeft: '4px solid #0275d8' }}>
        ...
        <div style={{ lineHeight: '1.6', fontFamily: 'sans-serif', whiteSpace: 'pre-wrap' }}>
            {/* Simple Markdown Bold parsing for the UI */}
            {insight.split('**').map((text, i) => i % 2 === 1 ? <strong key={i}>{text}</strong> : text)}
        </div>
      </div>
    </div>
  );
